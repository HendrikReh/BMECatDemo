...
def parse_product(data: dict) -> tuple[Product, list[ProductPrice], list[ProductMedia]]:
...
def import_jsonl(file_path: str) -> int:
    """
    ...
    with Session(sync_engine) as session:
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                product, prices, media_items = parse_product(data)
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python -m src.db.import_jsonl <file.jsonl>", file=sys.stderr)
        sys.exit(1)
